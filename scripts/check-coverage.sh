#!/usr/bin/env bash
# Measures coverage over non-UI source (everything under src/ except src/ui/) and
# fails if it's below the threshold. src/ui/ (JUCE Component painting/layout code)
# is excluded per CODING_STANDARDS.md — pixel-level GUI testing isn't practical.
#
...
# Determine whether any non-UI src/ file has coverage data at all *after* filtering,
...
if [[ "$TOTAL_LINES" == "0" ]]; then
    echo "No non-UI code under src/ (outside src/ui/) has coverage data yet — nothing to gate on."
